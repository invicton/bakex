...
**Version:** 0.3.0  
**Date:** 2026-04-09  
**Scope:** All testable layers of the BakeX platform
...
|---|---|---|---|
| BP-01 | Missing required field `bakex_version` | Profile dict without `bakex_version` | `ValidationError` raised |
...
| WH-05 | HMAC-SHA256 signature is correct | Mock `httpx.AsyncClient.post`, capture `X-BakeX-Signature` header | Header matches `hmac.new(secret, body, sha256).hexdigest()` |
...
| SEC-03 | Signature prefix is `sha256=` | `X-BakeX-Signature` header starts with `sha256=` |
...
|---|---|---|---|
| REG-01 | App starts without error | `uvicorn bakex.main:app` | Process exits 0 within 5 seconds of startup check |
...
from bakex.core import api_keys
...
# With coverage
pytest --cov=bakex --cov=plugins --cov-report=term-missing
```
...
|---|---|---|
| `bakex/core/blueprint.py` | ~90% | 95% |
| `bakex/core/parser.py` | ~85% | 95% |
| `bakex/openscap/parser.py` | ~80% | 90% |
| `bakex/plugins/` | ~75% | 90% |
| `bakex/core/api_keys.py` | 0% | 90% |
| `bakex/core/notifications.py` | 0% | 85% |
| `bakex/core/playbook_gen.py` | 0% | 80% |
| `bakex/api/auditor.py` | 0% | 80% |
| `bakex/api/pipeline.py` | 0% | 85% |
| `bakex/api/api_keys.py` | 0% | 90% |
| `bakex/api/webhooks.py` | 0% | 85% |
| **Overall** | **~35%** | **80%** |
...
- **AI Agent** (`bakex/core/agent.py`) — Requires a live Anthropic API key and produces non-deterministic output. Test prompt construction and context building only; mock the `anthropic` client.
