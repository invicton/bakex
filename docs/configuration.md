...
|---|---|---|
| `STATIM_ADMIN_TOKEN` | auto-generated | Login for the UI and API (HTTP Basic — any username, this as the password). If unset, a random token is generated on first startup and saved to `data/.admin_token`; check there (or the container logs) to log in. Set it explicitly for a stable login. |
| `STATIM_SECRET_KEY` | auto-generated | Passphrase for the credential-encryption key (Fernet). If unset, a random key is stored in `data/.statim_key`. Set it so onboarded cloud credentials survive container rebuilds. |
| `STATIM_AGENT_REQUIRE_CONFIRMATION` | `true` | Require a human click before the AI Builder provisions real cloud infrastructure. |
...
Pick one via `STATIM_LLM_PROVIDER`:

| Provider | `STATIM_LLM_PROVIDER` | Auth | Notes |
|---|---|---|---|
| Anthropic | `anthropic` | `ANTHROPIC_API_KEY` | Default; extended thinking on by default |
| OpenAI / compatible | `openai` | `STATIM_LLM_API_KEY` | Groq, Together, vLLM, LiteLLM, Fireworks |
...
```bash
STATIM_LLM_PROVIDER=anthropic   # anthropic | openai | ollama | bedrock
STATIM_LLM_MODEL=               # override the backend's default model
STATIM_LLM_API_KEY=             # for openai-compatible backends
STATIM_LLM_BASE_URL=            # any OpenAI-compatible endpoint (Ollama: http://localhost:11434/v1)
STATIM_LLM_THINKING=1           # extended thinking (Anthropic/Bedrock), 1=on 0=off
```
...
Statim aggregates blueprints from the community GitHub library, an optional
...
BLUEPRINT_STORE_S3_PREFIX=statim/
BLUEPRINT_STORE_S3_REGION=us-east-1
```

## System dependencies for real builds

Statim itself is pure Python, but hardening and scanning shell out to
...
- Ubuntu 22.04 has no `openscap-scanner` apt package in any channel (it first
  appears in 24.04) — scanning targets running 22.04 relies on Statim's
