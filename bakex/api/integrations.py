...
# KDF parameters.
# _LEGACY_KDF_SALT was a fixed constant shared by every BakeX install that
# set BAKEX_SECRET_KEY — since the salt is public (in the open-source repo),
...
# Installs now get a random salt generated at data_dir/.bakex_salt on first
...
_LEGACY_KDF_SALT = b"bakex-credential-store-v1"
...
class CredentialStore:
    ...
    1. ``secret_key`` argument (BAKEX_SECRET_KEY env var) — PBKDF2-derived,
       deterministic across restarts (no key file needed).
    2. Existing ``data_dir/.bakex_key`` — loaded from disk.
    3. Auto-generated random key written to ``data_dir/.bakex_key`` (chmod 600).
    ...
    def __init__(self, data_dir: Path, secret_key: str | None = None) -> None:
        ...
        self._key_file = data_dir / ".bakex_key"
        self._salt_file = data_dir / ".bakex_salt"
    ...
    def _get_or_create_salt(self) -> bytes:
        """Return a random salt persisted at data_dir/.bakex_salt, generating
        one on first use so each install derives a unique key from the same
        BAKEX_SECRET_KEY passphrase (see _LEGACY_KDF_SALT comment above)."""
    ...
    def _init_fernet(self, secret_key: str | None) -> Fernet:
        ...
        if secret_key:
            ...
            logger.info("Credential store: using PBKDF2-derived key from BAKEX_SECRET_KEY")
...
def _make_store() -> CredentialStore:
    from bakex.config import settings  # avoid circular import at module level

    return CredentialStore(
        data_dir=settings.data_dir,
        secret_key=settings.bakex_secret_key,
    )
...
def _aws_stack_import_html(creds: dict, outputs: dict) -> str:
    role_arn = html.escape(outputs.get("BakeXRoleArn", ""), quote=True)
    ...
    return (
        ...
        'placeholder="arn:aws:iam::123456789012:role/BakeXBuilderRole" '
        ...
        'placeholder="bakex-your-company-or-random-id" '
        'class="w-full bg-slate-900/50 border border-slate-700/80 rounded-lg px-4 py-2.5 text-sm text-white '
        "placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 "
        'transition-all font-mono" />'
        ...
        'placeholder="BakeXInstanceProfile" '
        'class="w-full bg-slate-900/50 border border-slate-700/80 rounded-lg px-4 py-2.5 text-sm text-white '
        "placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 "
        'transition-all font-mono" />'
    )
...
async def import_aws_stack_outputs(request: Request) -> str:
    ...
    try:
        ...
        required = {"BakeXRoleArn", "ExternalId", "InstanceProfileName"}
    ...
    if provider_name == "aws":
        ...
        try:
            ...
            if role_arn:
                ...
                assume_kwargs = {
                    "RoleArn": role_arn,
                    "RoleSessionName": "BakeXConnectionTest",
                }
                if external_id:
            ...
            if isinstance(exc, (NoCredentialsError, PartialCredentialsError)):
                ...
                return _err_html(
                    ...
                    "or attach an identity policy allowing sts:AssumeRole on the BakeX role."
                )
            else:
                label = "AWS Error"
...
async def _test_gcp(creds: dict) -> str:
    ...
    try:
        ...
        return _err_html("google-cloud-compute not installed. Install bakex[gcp].")
    try:
...
async def _test_azure(creds: dict) -> str:
    ...
    try:
        ...
        return _err_html("azure-identity / azure-mgmt-resource not installed. Install bakex[azure].")
    try:
...
async def _test_digitalocean(creds: dict) -> str:
    ...
    try:
        ...
        return _err_html("requests not installed. Install bakex[digitalocean].")
    try:
        resp = _requests.get(
...
async def _test_linode(creds: dict) -> str:
    ...
    try:
        ...
        return _err_html("requests not installed. Install bakex[linode].")
    try:
        resp = _requests.get(
...
async def _test_proxmox(creds: dict) -> str:
    ...
    try:
        ...
        return _err_html("proxmoxer not installed. Install bakex[proxmox].")
    try:
