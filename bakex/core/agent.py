...
"""BakeXBuildAgent — agentic AI build system with pluggable LLM backends.
...
LLM backend is selected by BAKEX_LLM_PROVIDER env var:
  anthropic (default) | openai | ollama | bedrock
See bakex/core/llm/factory.py for full env-var reference.
"""
...
# ---------------------------------------------------------------------------
# Tool implementations — call into BakeX's internal services directly
# ---------------------------------------------------------------------------


def _apply_required_defaults(raw: dict) -> dict:
    """Add bakex_version and kind defaults if missing — they have no schema default."""
    raw.setdefault("bakex_version", "1")
...
TOOLS: list[dict] = [
    {
        ...
        "description": "Validate a blueprint YAML string against the BakeX ComplianceProfile schema. Returns {valid: bool, error?: str, profile_name?, os?, provider?, benchmark?, tier?}",
        "input_schema": {
            "type": "object",
            "properties": {
...
async def _execute_tool(tool_name: str, tool_input: dict) -> Any:
    ...
    if tool_name == "start_build":
        ...
        if settings.bakex_agent_require_confirmation:
            return {
                "error": (
                    "start_build is disabled while BAKEX_AGENT_REQUIRE_CONFIRMATION is enabled "
                    ...
                    "BAKEX_AGENT_REQUIRE_CONFIRMATION=false to let the agent build directly."
                )
            }
...
SYSTEM_PROMPT = """\
You are BakeXBuildAgent, an autonomous OS hardening engineer embedded in the BakeX platform.
...
async def run_build_agent(
    ...
    """Run the BakeXBuildAgent agentic loop.
    ...
    """
    from bakex.core.llm import TextBlock, ToolUseBlock, get_backend
