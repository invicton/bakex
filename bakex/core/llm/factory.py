...
Control via env vars:
  ...
  BAKEX_LLM_PROVIDER  — anthropic (default) | openai | ollama | bedrock
  BAKEX_LLM_MODEL     — model override (provider-specific default if unset)
  BAKEX_LLM_API_KEY   — API key for openai-compatible backends
  BAKEX_LLM_BASE_URL  — base URL for openai-compatible backends
...
def get_backend() -> LLMBackend:
    provider = os.environ.get("BAKEX_LLM_PROVIDER", "anthropic").lower()
    ...
    if provider == "ollama":
        ...
        return OpenAICompatBackend(
            model=os.environ.get("BAKEX_LLM_MODEL", "llama3.3:70b"),
            base_url=os.environ.get("BAKEX_LLM_BASE_URL", "http://localhost:11434/v1"),
            api_key=os.environ.get("BAKEX_LLM_API_KEY", "ollama"),
        )
    ...
    raise ValueError(f"Unknown BAKEX_LLM_PROVIDER: {provider!r}. Valid values: anthropic, openai, ollama, bedrock")


def provider_status() -> dict:
    """Return availability info for the configured provider (used by /api/agent/status)."""
    provider = os.environ.get("BAKEX_LLM_PROVIDER", "anthropic").lower()
    model = os.environ.get("BAKEX_LLM_MODEL", "")
    ...
    if provider == "openai":
        has_key = bool(os.environ.get("BAKEX_LLM_API_KEY") or os.environ.get("OPENAI_API_KEY"))
        ...
        return {
            ...
            "message": (
                ...
                if has_key
                else "Set BAKEX_LLM_API_KEY (or OPENAI_API_KEY) in your .env file."
            ),
        }
    ...
    return {
        ...
        "provider": provider,
        "model": model,
        "message": f"Unknown BAKEX_LLM_PROVIDER: {provider!r}. Valid: anthropic, openai, ollama, bedrock",
    }
