...
Requires the [gcp] optional extra: pip install bakex[gcp]
...
def _get_compute_client(credentials: dict):
    ...
    try:
        ...
        raise RuntimeError("google-cloud-compute is not installed. Install with: pip install bakex[gcp]") from exc
...
def execute_build(params: dict) -> dict:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="bakex-gcp-") as tmpdir:
            tmp = Path(tmpdir)
            ...
            instance_name = f"bakex-build-{profile_name.lower()[:20]}-{int(time.time())}"
            ...
            config = compute_v1.Instance(
                ...
                labels={"managed-by": "bakex", "blueprint": profile_name.lower()[:60]},
            ...
            image_name = f"bakex-{profile_name.lower()[:40]}-{profile_version.replace('.', '-')}"
...
def execute_audit(params: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="bakex-gcp-audit-") as tmpdir:
...
def execute_scan_image(params: dict) -> dict:
    ...
    instance_name: str | None = None
    with tempfile.TemporaryDirectory(prefix="bakex-gcp-scan-") as tmpdir:
        tmp = Path(tmpdir)
        try:
            from google.cloud import compute_v1
            ...
            instance_name = f"bakex-scan-{int(time.time())}"
            config = compute_v1.Instance(
                name=instance_name,
                machine_type=f"zones/{zone}/machineTypes/{machine_type}",
                disks=[
                ...
                tags=compute_v1.Tags(items=["bakex-scan"]),
            )
            op = instances_client.insert(project=project_id, zone=zone, instance_resource=config)
