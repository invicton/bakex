...
Requires the [linode] optional extra: pip install stratum[linode]
...
def _get_client(token: str):
    try:
        ...
        raise RuntimeError("linode-api4 is not installed. Install with: pip install stratum[linode]") from exc
...
def execute_build(params: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="stratum-linode-") as tmpdir:
        tmp = Path(tmpdir)
        ...
        try:
            ...
            instance, _ = client.linode.instance_create(
                ...
                region=region,
                image=base_image,
                label=f"stratum-build-{profile_name[:20]}",
                root_pass=root_pass,
            ...
            image_label = f"stratum-{profile_name[:30]}-{profile_version}"
...
def execute_audit(params: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="stratum-linode-audit-") as tmpdir:
...
def execute_scan_image(params: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="stratum-linode-scan-") as tmpdir:
        tmp = Path(tmpdir)
        key_path, pub_key = utils.generate_ssh_keypair(tmp)
        try:
            ...
            instance, _ = client.linode.instance_create(
                ...
                region=region,
                image=image_id,
                label=f"stratum-scan-{int(time.time())}",
                root_pass=root_pass,
                authorized_keys=[pub_key],
