...
Requires the [digitalocean] optional extra: pip install stratum[digitalocean]
...
class DOClient:
    ...
    def __init__(self, token: str) -> None:
        try:
            ...
            raise RuntimeError("requests is not installed. Install with: pip install stratum[digitalocean]") from exc
...
def execute_build(params: dict) -> dict:
    ...
    Credential / param fields:
        ...
        prehard_playbook_yaml — Pre-hardening playbook YAML (from Stratum engine)
    ...
    with tempfile.TemporaryDirectory(prefix="stratum-do-") as tmpdir:
        tmp = Path(tmpdir)
        try:
            # 1. Generate ephemeral SSH key pair
            key_path, pub_key = utils.generate_ssh_keypair(tmp)
            key_name = f"stratum-build-{profile_name}-{int(time.time())}"
            ...
            droplet_body = {
                "name": f"stratum-build-{profile_name}",
                "region": region,
                "size": size_slug,
                "image": base_image,
            ...
            # 9. Create snapshot
            snap_name = f"stratum-{profile_name}-{profile_version}"
...
def execute_audit(params: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="stratum-do-audit-") as tmpdir:
...
def execute_scan_image(params: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="stratum-do-scan-") as tmpdir:
        tmp = Path(tmpdir)
        try:
            key_path, pub_key = utils.generate_ssh_keypair(tmp)
            key_name = f"stratum-scan-{int(time.time())}"
            temp_key_id = client.add_ssh_key(key_name, pub_key)

            droplet_body = {
                "name": f"stratum-scan-{int(time.time())}",
                "region": region,
                "size": size_slug,
                "image": image_id,
                "ssh_keys": [temp_key_id],
                "backups": False,
                "tags": ["stratum", "image-scan"],
            }
            resp = client.post("/droplets", droplet_body)
            droplet_id = resp["droplet"]["id"]
            droplet = client.wait_droplet_status(droplet_id, "active", timeout=300)
