...
Requires the [azure] optional extra: pip install bakex[azure]
...
def _get_credential(credentials: dict):
    try:
        ...
        raise RuntimeError("azure-identity not installed. Install with: pip install bakex[azure]") from exc
...
def execute_build(params: dict) -> dict:
    ...
    suffix = f"bakex{int(time.time())}"
    vm_name = f"bakex-bld-{suffix[:18]}"
    nic_name = f"bakex-nic-{suffix[:18]}"
    pip_name = f"bakex-pip-{suffix[:18]}"
    vnet_name = f"bakex-vnet-{suffix[:18]}"
    subnet_name = "bakex-subnet"
    disk_name = f"bakex-disk-{suffix[:18]}"
    captured_image_name = f"bakex-{profile_name.lower()[:30]}-{profile_version.replace('.', '-')}"

    resources_to_delete = []

    with tempfile.TemporaryDirectory(prefix="bakex-azure-") as tmpdir:
        tmp = Path(tmpdir)
        ...
        try:
            ...
            vm_params = VirtualMachine(
                ...
                tags={"managed-by": "bakex", "blueprint": profile_name},
            )
...
def execute_audit(params: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="bakex-az-audit-") as tmpdir:
