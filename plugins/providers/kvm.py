...
Builds hardened images on the machine running BakeX, no cloud account
...
BakeX provider uses. The output artifact is a qcow2 (optionally converted
...
Credential fields (stored via BakeX integrations UI — all optional, this
...
def execute_build(params: dict) -> dict:
    ...
    proc = None
    with tempfile.TemporaryDirectory(prefix="bakex-kvm-") as tmpdir:
        tmp = Path(tmpdir)
        key_path, pub_key = utils.generate_ssh_keypair(tmp)
        seed_iso = qemu.build_seed_iso(tmp, pub_key, ssh_user, hostname=f"bakex-{job_id[:8]}")
