...
Connectivity model:
  ...
  Option A — Public IP (default, works anywhere):
    ...
    Stratum connects over the internet (ensure firewall allows port 22
    from the Stratum host only — not 0.0.0.0/0).

  Option B — VPC private networking (recommended for production):
    Set credentials.use_private_ip: true
    The Stratum host must also be a Droplet in the same DigitalOcean
...
Requires the [digitalocean] optional extra:
    pip install 'stratum[digitalocean]'
...
def _do_api(token: str, method: str, path: str, **kwargs) -> dict | list | None:
    ...
    try:
        ...
        raise RuntimeError("requests is not installed. Run: pip install 'stratum[digitalocean]'") from exc
...
def _generate_ssh_keypair() -> tuple[str, str, str]:
    ...
    tmp = tempfile.mkdtemp(prefix="stratum-do-key-")
...
def execute_build(params: dict) -> dict:
    ...
    Set credentials.use_private_ip: true if the Stratum host is a Droplet
    ...
    droplet_name = f"stratum-{safe_name}-{int(time.time())}"
    ...
    try:
        ...
        key_label = f"stratum-build-{int(time.time())}"
        ...
        snapshot_name = f"stratum-{safe_name}-{safe_version}"
