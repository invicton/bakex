...
Connectivity model (no public IP required):
  Stratum host → Cloud IAP TCP tunnel → GCE instance (private subnet)
...
Requirements on the Stratum host:
...
Requirements on the GCP project:
  - VPC firewall rule allowing IAP to reach port 22 on the build VMs:
      Source: 35.235.240.0/20  →  Target: tag stratum-build  →  TCP 22

Requires the [gcp] optional extra:
    pip install 'stratum[gcp]'
...
def _generate_ssh_keypair() -> tuple[str, str, str]:
    ...
    tmp = tempfile.mkdtemp(prefix="stratum-gcp-key-")
...
def execute_build(params: dict) -> dict:
    ...
    Flow:
      1. Launch private GCE instance (no external IP, tag: stratum-build)
    ...
    try:
        ...
        raise RuntimeError("google-cloud-compute is not installed. Run: pip install 'stratum[gcp]'") from exc
    ...
    instance_name = f"stratum-{safe_name}-{int(time.time())}"
    ...
    try:
        ...
        instance_resource = compute_v1.Instance(
            ...
            metadata=compute_v1.Metadata(
                items=[
                    compute_v1.Items(key="ssh-keys", value=f"stratum_build:{pub_key}"),
        ...
        _wait_for_ssh_via_tunnel(local_port, key_path, "stratum_build", timeout=300)
        ...
        if prehard_yaml:
            ...
            _run_ansible_yaml_via_tunnel(prehard_yaml, local_port, key_path, "stratum_build", tmp_dir)
        ...
        oscap_cmd = (
            f"sudo oscap xccdf eval --profile {profile_id} --results /tmp/stratum-scap-results.xml {datastream} || true"
        )
        _run_remote_cmd_via_tunnel(local_port, key_path, "stratum_build", oscap_cmd, timeout=600)
        ...
        try:
            _run_remote_cmd_via_tunnel(local_port, key_path, "stratum_build", " ; ".join(cleanup_cmds), timeout=120)
        except Exception as exc:
        ...
        image_name = f"stratum-{safe_name}-{safe_version}"[:63].lower()
