...
"""AWS Bedrock Converse API backend for the Stratum agentic build system.

Uses existing AWS credentials (same creds Stratum uses for EC2/AMI operations).
No additional API key required — just needs bedrock:InvokeModelWithResponseStream
permission on the target model.

Env vars:
  STRATUM_LLM_MODEL     — Bedrock model ID
...
class BedrockBackend:
    ...
    def __init__(self, model: str | None = None, region: str | None = None) -> None:
        self.model = model or os.environ.get("STRATUM_LLM_MODEL", self.DEFAULT_MODEL)
