...
from stratum.core.blueprint import ComplianceProfile, MountEntry, SystemConfig, UsersConfig
...
def generate_prehard_playbook(profile: ComplianceProfile) -> Path | None:
    ...
    play = {
        "name": f"Stratum pre-hardening system configuration — {profile.metadata.name} v{profile.metadata.version}",
        "hosts": "all",
        "become": True,
        "gather_facts": True,
        "tasks": tasks,
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="stratum-playbook-"))
