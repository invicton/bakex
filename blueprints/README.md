...
Stratum keeps built-in runtime templates in `profiles/templates/`. Community contributions
...
- Keep `stratum_version` and `kind: HardeningBlueprint`.
...
- **CIS L2 for existing OSes.** Rocky 9, RHEL 9, Amazon Linux 2023, and Debian 12 all have CIS L2
  support in Stratum's `OS_CATALOG` already (SCAP profile + Ansible-Lockdown role variables both
  exist) — there just isn't a blueprint file for it yet in this library.
- **RHEL 9** blueprint (the OS is fully supported by Stratum; no blueprint file exists here yet).
