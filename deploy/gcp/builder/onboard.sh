...
ROLE_ID="${ROLE_ID:-stratumBuilder}"
SERVICE_ACCOUNT_ID="${SERVICE_ACCOUNT_ID:-stratum-builder}"
...
if [[ -z "${PRINCIPAL}" ]]; then
  if ! gcloud iam service-accounts describe "${SERVICE_ACCOUNT_ID}@${PROJECT_ID}.iam.gserviceaccount.com" --project "${PROJECT_ID}" >/dev/null 2>&1; then
    gcloud iam service-accounts create "${SERVICE_ACCOUNT_ID}" \
      --project "${PROJECT_ID}" \
      --display-name "Stratum builder service account"
  fi
...
if gcloud iam roles describe "${ROLE_ID}" --project "${PROJECT_ID}" >/dev/null 2>&1; then
  gcloud iam roles update "${ROLE_ID}" --project "${PROJECT_ID}" --file "${SCRIPT_DIR}/stratum-builder-role.yaml"
else
  gcloud iam roles create "${ROLE_ID}" --project "${PROJECT_ID}" --file "${SCRIPT_DIR}/stratum-builder-role.yaml"
fi
...
cat <<EOF
Stratum GCP builder onboarding complete.
