...
BakeX uses native `gcloud` onboarding scripts and explicit custom-role YAML files for GCP. There is no third-party IaC dependency.
...
```bash
PROJECT_ID=my-gcp-project \
PRINCIPAL=serviceAccount:bakex@example.iam.gserviceaccount.com \
./onboard.sh
```
...
- The IAP firewall rule allows TCP 22 only from `35.235.240.0/20` to instances tagged `bakex-build` or `bakex-scan`.
- Customers can inspect `bakex-scanner-role.yaml` and `bakex-builder-role.yaml` to review every granted permission before applying.
