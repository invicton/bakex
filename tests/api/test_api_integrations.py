...
def test_download_aws_template_serves_bundled_cloudformation(client):
    resp = client.get("/api/integrations/aws/templates/bakex-scanner-role.yaml")
    assert resp.status_code == 200
...
def test_import_aws_stack_outputs_saves_cloudformation_outputs(client):
        ...
        "Stacks": [
            {
                "Outputs": [
                    {
                        "OutputKey": "BakeXRoleArn",
                        "OutputValue": "arn:aws:iam::123456789012:role/BakeXBuilderRole",
                    },
                    {"OutputKey": "ExternalId", "OutputValue": "bakex-onboarding"},
                    {"OutputKey": "InstanceProfileName", "OutputValue": "BakeXBuilderInstanceProfile"},
    ...
    with patch.dict("sys.modules", {"boto3": mock_boto3, "botocore": MagicMock(), "botocore.exceptions": MagicMock()}):
        resp = client.post(
            ...
            data={
                "cloudformation_stack_name": "bakex-builder",
    ...
    mock_cfn.describe_stacks.assert_called_once_with(StackName="bakex-builder")
    saved = client.get("/api/integrations/aws").json()
    assert saved["role_arn"] == "arn:aws:iam::123456789012:role/BakeXBuilderRole"
    assert saved["external_id"] == "bakex-onboarding"
    assert saved["iam_profile_name"] == "BakeXBuilderInstanceProfile"
...
def test_test_credentials_aws_success(client):
    ...
    with patch.dict("sys.modules", {"boto3": mock_boto3, "botocore": MagicMock(), "botocore.exceptions": MagicMock()}):
        ...
        with patch("bakex.api.integrations.boto3", mock_boto3, create=True):
            resp = client.post(
...
def test_err_html_escapes_special_chars(client):
    ...
    from bakex.api.integrations import _err_html
...
def test_ok_html_contains_message(client):
    from bakex.api.integrations import _ok_html
...
def test_test_credentials_gcp_success(client):
    ...
    with patch.dict(
        ...
        with patch("bakex.api.integrations.compute_v1", mock_compute, create=True):
            resp = client.post(
                "/api/integrations/gcp/test",
                data={"project_id": "my-proj"},
            )
...
def test_test_credentials_gcp_exception(client):
    ...
    with patch("bakex.api.integrations._test_gcp", side_effect=_fail):
        resp = client.post(
            "/api/integrations/gcp/test",
            data={"project_id": "my-proj"},
        )
...
def test_test_credentials_azure_success(client):
    ...
    with patch.dict(
        ...
        with patch("bakex.api.integrations._test_azure") as mock_fn:
...
def test_test_credentials_digitalocean_success(client):
    ...
    with patch("bakex.api.integrations._test_digitalocean") as mock_fn:
...
def test_test_credentials_linode_success(client):
    with patch("bakex.api.integrations._test_linode") as mock_fn:
...
def test_test_credentials_proxmox_success(client):
    with patch("bakex.api.integrations._test_proxmox") as mock_fn:
...
def test_credential_store_delete(client):
    ...
    from bakex.api.integrations import credential_store
...
async def test_gcp_helper_missing_project_id():
    from bakex.api.integrations import _test_gcp
...
async def test_azure_helper_missing_tenant():
    from bakex.api.integrations import _test_azure
...
async def test_digitalocean_helper_missing_token():
    from bakex.api.integrations import _test_digitalocean
...
async def test_linode_helper_missing_token():
    from bakex.api.integrations import _test_linode
...
async def test_proxmox_helper_missing_host():
    from bakex.api.integrations import _test_proxmox
