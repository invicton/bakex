...
def test_blueprints_page_load_profile_raises(client, profiles_tmp, monkeypatch):
    ...
    with patch("bakex.api.ui.load_profile", side_effect=ValueError("bad yaml")):
        resp = client.get("/blueprints")
    assert resp.status_code == 200
...
def test_blueprints_page_registry_list_raises(client, profiles_tmp, monkeypatch):
    ...
    with patch("bakex.api.ui.load_profile", side_effect=ValueError("skip")):
        with patch("bakex.core.registry.get_registry", return_value=mock_reg):
            resp = client.get("/blueprints")
    assert resp.status_code == 200
...
def test_blueprint_studio_load_profile_raises(client, profiles_tmp, monkeypatch):
    ...
    monkeypatch.setattr(settings, "profiles_dir", profiles_tmp)
    with patch("bakex.api.ui.load_profile", side_effect=ValueError("bad")):
        with patch("bakex.core.registry.get_registry") as mock_get:
...
def test_download_blueprint_load_profile_raises(client, profiles_tmp, monkeypatch):
    ...
    monkeypatch.setattr(settings, "profiles_dir", profiles_tmp)
    with patch("bakex.api.blueprints.load_profile", side_effect=ValueError("bad")):
        with patch("bakex.core.registry.get_registry") as mock_get:
            mock_get.return_value.get.return_value = None
...
def test_compare_scans_findings_format(client):
    ...
    with patch("bakex.openscap.parser.compute_delta", return_value={"added": [], "removed": [], "changed": []}):
...
def test_audit_resolve_profile_load_raises(client, profiles_tmp, monkeypatch):
    ...
    monkeypatch.setattr(settings, "profiles_dir", profiles_tmp)
    with patch("bakex.api.auditor.load_profile", side_effect=ValueError("bad")):
        resp = client.post(
            "/api/auditor/start",
