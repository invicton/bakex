...
def test_pipeline_build_wait_true_returns_complete(client, api_key):
    ...
    with patch("bakex.api.pipeline.build_service.run_build", side_effect=_fake_run):
        resp = client.post(
            "/api/pipeline/build",
...
# raise (TargetSpec has no `region` field — see bakex/api/pipeline.py).
# ---------------------------------------------------------------------------


def test_pipeline_build_with_region_does_not_raise(client, api_key):
    from bakex.core.builder import BuildStatus
    from bakex.plugins.base_provider import ProviderResult
...
def test_pipeline_build_wait_false_returns_pending(client, api_key):
    ...
    with (
        patch("bakex.api.pipeline.build_service.run_build", new_callable=AsyncMock),
        patch("bakex.api.pipeline.asyncio.create_task", side_effect=close_scheduled),
    ):
        resp = client.post(
            "/api/pipeline/build",
