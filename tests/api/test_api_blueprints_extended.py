...
def test_download_blueprint_falls_back_to_community_registry(client, monkeypatch):
    ...
    with patch("bakex.core.registry.get_registry", return_value=mock_registry):
...
def test_find_profile_falls_back_to_community_registry():
    ...
    from bakex.api.blueprints import _find_profile
    from bakex.core.blueprint import ComplianceProfile
...
def test_find_profile_registry_runtime_error_returns_none():
    ...
    with patch("bakex.core.registry.get_registry", side_effect=RuntimeError("registry not ready")):
