...
from stratum.core.agent import AgentResult
...
def test_agent_build_provider_unavailable_returns_400(client):
    ...
    with patch("stratum.api.agent.provider_status", return_value={"available": False, "message": "No API key"}):
...
def test_agent_build_empty_yaml_returns_400(client):
    ...
    with patch("stratum.api.agent.provider_status", return_value={"available": True, "message": "ok"}):
...
async def test_agent_build_sse_streams_narration_and_result(client):
    ...
    with patch("stratum.api.agent.provider_status", return_value={"available": True, "message": "ok"}):
        with patch("stratum.api.agent.run_build_agent", side_effect=fake_run_build_agent):
            resp = client.post(
                "/api/agent/build",
...
async def test_agent_build_sse_streams_error_on_exception(client):
    ...
    with patch("stratum.api.agent.provider_status", return_value={"available": True, "message": "ok"}):
        with patch("stratum.api.agent.run_build_agent", side_effect=failing_agent):
            resp = client.post(
                "/api/agent/build",
                json={"blueprint_yaml": _VALID_YAML, "provider": "aws"},
            )
...
def test_agent_status_returns_provider_info(client):
    ...
    with patch(
        "stratum.api.agent.provider_status", return_value={"available": True, "provider": "anthropic", "message": "ok"}
    ):
