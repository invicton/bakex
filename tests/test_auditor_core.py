...
import stratum.core.auditor as auditor_mod
from stratum.core.auditor import (
    AuditJob,
    AuditStatus,
    _severity_counts,
    get_audit,
...
async def test_run_image_scan_cloud_path_success(tmp_path):
    ...
    with patch.object(registry, "get", return_value=mock_provider_cls):
        with patch("stratum.core.parser.SCAPParser.parse_report", return_value=mock_results):
            job = await auditor_mod.run_image_scan(
                image_id="ami-00001",
                provider_name="aws",
                region="us-east-1",
...
async def test_run_image_scan_ssh_path_success(tmp_path):
    ...
    with patch.object(registry, "get", return_value=mock_provider_cls):
        with patch("stratum.core.auditor.oscap_scanner.run_scan", return_value=fake_arf):
            with patch("stratum.core.auditor.parse_arf", return_value=mock_results):
                job = await auditor_mod.run_image_scan(
                    image_id="ami-ssh",
                    provider_name="local",
                    region="",
...
async def test_run_image_scan_ssh_path_teardown_on_failure(tmp_path):
    ...
    with patch.object(registry, "get", return_value=mock_provider_cls):
        with patch("stratum.core.auditor.oscap_scanner.run_scan", side_effect=RuntimeError("scan failed")):
            job = await auditor_mod.run_image_scan(
                image_id="ami-fail",
                provider_name="local",
                region="",
...
async def test_run_image_scan_ssh_path_teardown_exception_ignored(tmp_path):
    ...
    with patch.object(registry, "get", return_value=mock_provider_cls):
        with patch("stratum.core.auditor.oscap_scanner.run_scan", side_effect=RuntimeError("scan err")):
            job = await auditor_mod.run_image_scan(
                image_id="ami-xyz",
                provider_name="local",
                region="",
...
async def test_run_audit_local_path_with_baseline(tmp_path):
    ...
    with patch.object(registry, "get", return_value=mock_provider_cls):
        ...
        with patch("stratum.core.auditor.oscap_scanner.run_scan", return_value=fake_arf):
            with patch("stratum.core.auditor.parse_arf", return_value=mock_results):
                with patch("stratum.core.auditor.compute_delta", return_value=mock_delta):
                    job = await auditor_mod.run_audit(
                        profile=_make_minimal_profile(),
...
async def test_run_audit_exception_marks_failed(tmp_path):
    ...
    mock_provider_cls.handles_full_lifecycle = False
    with patch.object(registry, "get", return_value=mock_provider_cls):
        with patch("stratum.core.auditor.oscap_scanner.run_scan", side_effect=RuntimeError("oscap not found")):
            job = await auditor_mod.run_audit(
                profile=_make_minimal_profile(),
...
def test_persist_jobs_exception_is_warned(tmp_path, monkeypatch, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="stratum.core.auditor"):
