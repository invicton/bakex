...
"""Unit tests for bakex.core.notifications — Phase 1 TDD red run."""
...
async def test_fire_webhook_hmac_signature_correct():
    ...
    assert "X-BakeX-Signature" in captured_headers, "X-BakeX-Signature header missing"

    sig_header = captured_headers["X-BakeX-Signature"]
...
# ---------------------------------------------------------------------------
# Extra: X-BakeX-Event header matches fired event
# ---------------------------------------------------------------------------
...
async def test_fire_webhook_event_header_matches():
    ...
    assert captured_headers.get("X-BakeX-Event") == "build.complete"
...
async def test_fire_webhook_noop_when_no_matching_webhooks():
    ...
    with patch("bakex.core.notifications.httpx", side_effect=AssertionError("httpx must not be called")):
        await notif_mod.fire_webhook("scan.complete", {"job_id": "x"})
