...
Covers the previously uncovered paths in:
  - stratum/core/llm/anthropic_backend.py  (was 28%)
  - stratum/core/llm/openai_backend.py     (was 35%)
  - stratum/core/llm/bedrock_backend.py    (was 43%)
  - stratum/core/llm/factory.py            (was 97%)
"""
...
from stratum.core.llm.base import TextBlock, ToolUseBlock
from stratum.core.llm.bedrock_backend import (
    BedrockBackend,
...
class TestFactory:
    def test_provider_status_anthropic_no_key(self, monkeypatch):
        ...
        monkeypatch.setenv("STRATUM_LLM_PROVIDER", "anthropic")
        from stratum.core.llm.factory import provider_status
    ...
    def test_provider_status_openai_no_key(self, monkeypatch):
        monkeypatch.delenv("STRATUM_LLM_API_KEY", raising=False)
        monkeypatch.delenv("OPENAI_API_KEY", raising=False)
        monkeypatch.setenv("STRATUM_LLM_PROVIDER", "openai")
        from stratum.core.llm.factory import provider_status
    ...
    def test_provider_status_openai_with_key(self, monkeypatch):
        monkeypatch.setenv("STRATUM_LLM_API_KEY", "sk-test")
        monkeypatch.setenv("STRATUM_LLM_PROVIDER", "openai")
        from stratum.core.llm.factory import provider_status
    ...
    def test_provider_status_ollama_always_available(self, monkeypatch):
        monkeypatch.setenv("STRATUM_LLM_PROVIDER", "ollama")
        from stratum.core.llm.factory import provider_status
    ...
    def test_provider_status_bedrock_with_key(self, monkeypatch):
        monkeypatch.setenv("STRATUM_LLM_PROVIDER", "bedrock")
    ...
    def test_provider_status_unknown_provider(self, monkeypatch):
        monkeypatch.setenv("STRATUM_LLM_PROVIDER", "groq")
        from stratum.core.llm.factory import provider_status
    ...
    def test_get_backend_anthropic(self, monkeypatch):
        monkeypatch.setenv("STRATUM_LLM_PROVIDER", "anthropic")
        from stratum.core.llm.anthropic_backend import AnthropicBackend
        from stratum.core.llm.factory import get_backend
    ...
    def test_get_backend_openai(self, monkeypatch):
        monkeypatch.setenv("STRATUM_LLM_PROVIDER", "openai")
        from stratum.core.llm.factory import get_backend
        from stratum.core.llm.openai_backend import OpenAICompatBackend
    ...
    def test_get_backend_bedrock(self, monkeypatch):
        monkeypatch.setenv("STRATUM_LLM_PROVIDER", "bedrock")
        from stratum.core.llm.bedrock_backend import BedrockBackend
        from stratum.core.llm.factory import get_backend
    ...
    def test_get_backend_unknown_raises(self, monkeypatch):
        monkeypatch.setenv("STRATUM_LLM_PROVIDER", "invalid_provider")
        from stratum.core.llm.factory import get_backend

        with pytest.raises(ValueError, match="Unknown STRATUM_LLM_PROVIDER"):
            get_backend()
