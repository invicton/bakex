...
"""Playwright browser smoke tests for all BakeX UI pages.

These tests run against the live BakeX server at http://localhost:8001.
They require:
...
All tests are skipped automatically when BAKEX_RUN_UI is not set.
...
pytestmark = [
    pytest.mark.ui,
    pytest.mark.skipif(
        not os.environ.get("BAKEX_RUN_UI"),
        reason="Set BAKEX_RUN_UI=1 to run Playwright UI tests",
    ),
]
...
def test_dashboard_title(page: Page):
    _goto(page, "/")
    expect(page).to_have_title("Dashboard — BakeX")
...
def test_blueprints_page_title(page: Page):
    ...
    expect(page).to_have_title("Profile Hub — BakeX")
...
def test_blueprint_studio_loads(page: Page):
    ...
    assert "BakeX" in page.title() or page.locator("body").is_visible()
...
def test_builder_page_title(page: Page):
    _goto(page, "/builder")
    expect(page).to_have_title("Image Builder — BakeX")
...
def test_auditor_page_title(page: Page):
    _goto(page, "/auditor")
    expect(page).to_have_title("Auditor — BakeX")
...
def test_scanner_wizard_title(page: Page):
    ...
    expect(page).to_have_title("Compliance Scanner — BakeX")
...
def test_scan_history_page_title(page: Page):
    ...
    expect(page).to_have_title("Scan History — BakeX")
...
def test_integrations_page_title(page: Page):
    ...
    expect(page).to_have_title("Integrations — BakeX")
...
def test_agent_page_title(page: Page):
    _goto(page, "/agent")
    expect(page).to_have_title("AI Builder — BakeX")
...
def test_api_keys_page_title(page: Page):
    ...
    expect(page).to_have_title("API Keys — BakeX")
...
def test_webhooks_page_title(page: Page):
    ...
    expect(page).to_have_title("Webhooks — BakeX")
