...
from stratum.core.parser import SCAPParser as XCCDFParser
...
class TestRegistrySync:
    ...
    async def test_sync_unknown_source_kind_emits_warning(self, caplog):
        ...
        reg = self._make_registry(src)
        with caplog.at_level(logging.WARNING, logger="stratum.core.registry"):
            names = await reg.sync()
        assert names == []
    ...
    async def test_sync_github_per_file_failure_logged(self, caplog):
        ...
        with patch("stratum.core.registry.httpx.AsyncClient", return_value=mock_client):
            with caplog.at_level(logging.WARNING, logger="stratum.core.registry"):
                names = await reg.sync()
