...
"""Tests for stratum/core/agent.py tool implementations and agent loop.
...
async def test_start_build_creates_job():
    with patch("stratum.core.agent.asyncio.create_task"):
...
async def test_run_build_agent_end_turn_success():
    ...
    from stratum.core.llm.base import AgentTurnResult, TextBlock
    ...
    with patch("stratum.core.llm.get_backend", return_value=mock_backend):
...
async def test_run_build_agent_backend_unavailable_returns_error():
    ...
    with patch("stratum.core.llm.get_backend", side_effect=RuntimeError("No API key")):
        result = await run_build_agent(_VALID_YAML, "aws", on_token)
...
async def test_run_build_agent_unexpected_stop_reason_returns_error():
    ...
    from stratum.core.llm.base import AgentTurnResult
...
async def test_run_build_agent_tool_exception_is_caught():
    ...
    with (
        patch("stratum.core.llm.get_backend", return_value=mock_backend),
        patch("stratum.core.agent._execute_tool", side_effect=RuntimeError("cloud error")),
    ):
        result = await run_build_agent(_VALID_YAML, "aws", on_token)
...
async def test_run_build_agent_records_job_id_from_start_build():
    ...
    with (
        patch("stratum.core.llm.get_backend", return_value=mock_backend),
        patch("stratum.core.agent._execute_tool", return_value={"job_id": "jb-abc-123", "status": "pending"}),
    ):
        result = await run_build_agent(_VALID_YAML, "aws", on_token)
