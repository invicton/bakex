...
from stratum.core.builder import BuildJob, BuildStatus, get_job, list_jobs, run_build
...
async def test_run_build_local_complete(tmp_path):
    ...
    with patch.object(builder_mod.registry, "get", return_value=cls):
        with patch("stratum.core.builder.generate_prehard_playbook", return_value=None):
            with patch("stratum.core.builder.oscap_scanner.run_scan", return_value=fake_arf):
                with patch("stratum.openscap.parser.parse_arf", return_value={"rules": []}):
                    job = await run_build(profile, tmp_path)
...
async def test_run_build_local_runs_prehard_playbook(tmp_path):
    ...
    with patch.object(builder_mod.registry, "get", return_value=cls):
        with patch("stratum.core.builder.generate_prehard_playbook", return_value=fake_playbook):
            with patch("stratum.core.builder.oscap_scanner.run_scan", return_value=fake_arf):
                with patch("stratum.core.builder._run_prehard_ansible") as mock_prehard:
                    with patch("stratum.openscap.parser.parse_arf", return_value={"rules": []}):
                        job = await run_build(profile, tmp_path)
