...
"""Trigger BakeX official blueprint builds via the REST API.
...
BAKEX_API_URL = os.environ.get("BAKEX_API_URL", "http://localhost:8000")
...
def trigger_build(client: httpx.Client, profile_name: str) -> str | None:
    ...
    provider = os.environ.get("BAKEX_BUILD_PROVIDER", "aws")
    region = os.environ.get("BAKEX_BUILD_REGION", os.environ.get("AWS_DEFAULT_REGION", "us-east-1"))

    try:
        resp = client.post(
            f"{BAKEX_API_URL}/api/builder/start",
...
def poll_job(client: httpx.Client, job_id: str, profile_name: str) -> str:
    ...
    while time.time() < deadline:
        try:
            resp = client.get(f"{BAKEX_API_URL}/api/builder/jobs/{job_id}", timeout=15)
            resp.raise_for_status()
            data = resp.json()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Trigger BakeX official blueprint builds")
    ...
    print(f"Target API: {BAKEX_API_URL}")
