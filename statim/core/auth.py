...
Statim is a single-operator, self-hosted tool with no reverse proxy assumed
...
check, see ``statim.api.pipeline``) is gated by one of the two dependencies
below:
...
_UNAUTHORIZED = HTTPException(
    ...
    headers={"WWW-Authenticate": 'Basic realm="Statim"'},
)
...
def get_admin_token() -> str:
    ...
    try:
        ...
        pass
    logger.warning(
        "No STATIM_ADMIN_TOKEN set — generated an admin token and saved it to %s. "
